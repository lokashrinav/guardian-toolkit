...
/**
 * Fixed MCP Server for Baseline Web Features
...
class MCPProtocol {
                ...
                for (const line of lines) {
                    if (line.trim()) {
                        try {
                            ...
                            this.sendError(-32700, 'Parse error: ' + parseError.message, null);
                        }
                    }
                }
            } catch (error) {
    ...
    async handleMessage(message) {
        try {
            if (!message.method) {
                this.sendError(-32600, 'Invalid Request', message.id || null);
                return;
            }
            ...
            switch (message.method) {
                ...
                case 'tools/call':
                    if (!message.params || !message.params.name) {
                        this.sendError(-32602, 'Invalid params', message.id);
                        break;
                    }
            ...
            this.sendError(-32603, 'Internal error: ' + error.message, message.id || null);
        }
    }
...
if (require.main === module) {
    // Remove the stderr logging that interferes with JSON-RPC
    new MCPProtocol();
}
